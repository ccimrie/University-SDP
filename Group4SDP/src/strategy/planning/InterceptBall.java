	...
	public void run() {
		...
		try {
			...
			while (!shouldidie && !Strategy.alldie) {
				...
				if (DistanceCalculator.Distance(world.ourRobot.x,
						world.ourRobot.y, projBallPos.getX(),
						projBallPos.getY()) > distanceThreshold) {
				...
				if (world.whoHasTheBall() == ourRobot)
					break;
			}
			...
			if (Math.abs(ball.x - world.getOurGoal().getX()) > Math.abs(us.x
				...
				mover.move(0, 100);
				while (world.whoHasTheBall() != ourRobot && !shouldidie
						&& !Strategy.alldie) {
					SafeSleep.sleep(50);
				}
				mover.stopRobot();
