...
//	private int[] yDistortion;
	...
	public void processAndUpdateImage(BufferedImage image, long before,
		...
		int topBuffer = pitchConstants.topBuffer - 10;
...
//				row = actualY;
		...
		for (int i = 0; i < ballXPoints.size(); i++)
			image.getGraphics().drawOval(ballXPoints.get(i), ballYPoints.get(i), 3, 3);
		
		try {
			float blueOrientation = findOrient(image, blue, blueXPoints, blueYPoints, 120, 500);
			
			float diff = Math.abs(blueOrientation - worldState.getBlueOrientation());
			if (diff > 0.1) {
				double angle = Math.round(180 * blueOrientation / Math.PI);//(float) Math.round(((blueOrientation / Math.PI) * 180) / 5) * 5;
		...
		try {
			float yellowOrientation = findOrient(image, yellow, yellowXPoints, yellowYPoints, 120, 500);
		...
		original = new int[]{ball.getX(), ball.getY()};
		corrected = convertToBarrelCorrected(original);
		worldState.setBallX(corrected[0]);
		worldState.setBallY(corrected[1]);
		
		original = new int[]{blue.getX(), blue.getY()};
		corrected = convertToBarrelCorrected(original);
		worldState.setBlueX(corrected[0]);
		worldState.setBlueY(corrected[1]);
		
		original = new int[]{yellow.getX(), yellow.getY()};
		corrected = convertToBarrelCorrected(original);
		worldState.setYellowX(corrected[0]);
		worldState.setYellowY(corrected[1]);*/
...
//	}
	...
	public int[] convertToBarrelCorrected(int[] p1) {
		...
		double py = (2 * p1[1] - height * 1.005) / (double) height;
...
//		double length = Math.sqrt(Position.sqrdEuclidDist(	centroid.getX(), centroid.getY(),
//															finalPoint.getX(), finalPoint.getY()));
	...
	public float findOrientation(ArrayList<Integer> xpoints,
		...
		float length = (float) Math.sqrt(Math.pow(frontX - meanX, 2) + Math.pow(frontY - meanY, 2));
...
//		}
		...
		length = (float) Math.sqrt(Math.pow(meanX - backX, 2) + Math.pow(meanY - backY, 2));
