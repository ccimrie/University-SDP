...
import simulator.old.SimServer;
import simulator.old.SimWorld;
import simulator.old.SimulatorRobot;
...
public class RoboFootball extends TestbedTest {
	...
	boolean ourRoboKick = false, ourRoboTurnLeft = false,
			ourRoboTurnRight = false, ourRoboGoForward = false,
			ourRoboGoBackwards = false;
	long kickStep = 0;
	...
	public synchronized void step(TestbedSettings settings) {
		...
		world.update(enemyRobo.robot.getWorldCenter().x / scale,
				...
				enemyRobo.robot.getAngle(), ourRobo.robot.getWorldCenter().x
						/ scale, ourRobo.robot.getWorldCenter().y / scale,
				ourRobo.robot.getAngle(), ball.getWorldCenter().x / scale,
	...
	public void executeCommand(byte newCommand, float arg) {
		...
		if (newCommand == Protocol.STOP) {
			...
			Vec2 p = enemyRobo.robot.getWorldPoint(enemyRobo.robot
					.getLocalCenter());
	...
	public void applyCommands() {
		if (isMoving) {
			if (fixedDistance) {
				if (distToGo > 0) {
					Vec2 i = enemyRobo.robot.getWorldVector(new Vec2(speed
							* enemyRobo.robot.getMass(), 0.0f));
					Vec2 p = enemyRobo.robot.getWorldPoint(enemyRobo.robot
							.getLocalCenter());
					...
					Vec2 i = enemyRobo.robot
							.getWorldVector(new Vec2(0.0f, 0.0f));
					Vec2 p = enemyRobo.robot.getWorldPoint(enemyRobo.robot
							.getLocalCenter());
		...
		if (isArcMoving) {
			...
			enemyRobo.robot.applyAngularImpulse(-(speed / radius)
					* enemyRobo.robot.getInertia() * correctionFactor);
		...
		if (isRotating && rotatingRight) {
			if (angleToGo > 0) {
				...
				enemyRobo.robot.applyAngularImpulse(-angVel
						* enemyRobo.robot.getInertia() * correctionFactor);
